...
包括的な互換性マトリクス、検証済みパターン、既知の制約については以下を参照:
...
**元の数値の出自: 未解明です。** この数値は 2026 年 5 月に本ドキュメントへ入ったもので、プロダクトレベルの特性として記載されていました。**ただし測定記録も測定条件も残されていません。** 比較する対象が存在しないため、差異の原因は特定できません。可能性として考えられる要因を列挙するに留めます: CLI ラッパー経由の測定でプロセス起動時間が短時間の呼び出しを支配していた、当時のファイルシステムが劣化した状態だった、あるいは当時から今回の測定までの間にプラットフォーム側の変更があった。
