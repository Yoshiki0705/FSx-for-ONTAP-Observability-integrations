...
| **問い 2 の状態** | 測定完了 |
| **問い 3 の状態** | 測定継続中（本ページ記載時点の経過は約 0.8 時間） |
...
測定は継続中である。本ページ記載時点で確定しているのは次の点だけである。

| 項目 | 値 |
|------|-----|
| 観測窓の起点 | `2026-08-25T17:16:53Z`（scope 変更に伴う再接続の完了時刻。それ以前の窓は無効） |
| 記載時点の経過 | 約 0.5 時間 |
| 72 時間到達予定 | `2026-08-28T17:16:53Z` |
| 自発的な切断 | 記載時点で 0 件 |
| サーバ側アイドルタイムアウト | 0 件 |
| KeepAlive 間隔の実測 | 120.4〜120.5 秒（エンジン設定 `keep_alive_interval=PT2M` と一致） |

この経過時間では「切断されない」と書けない。答えられるのは 72 時間の窓を満たした後である。
