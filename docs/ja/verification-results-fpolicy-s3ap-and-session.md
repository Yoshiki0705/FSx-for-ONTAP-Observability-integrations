...
| 根拠 | 内容 |
|------|------|
| 構造的な根拠 | ONTAP 9.18.1P3D1 の FPolicy event が受け付ける `protocol` 値は `cifs` / `nfsv3` / `nfsv4` の 3 つだけで、S3 やオブジェクトアクセスに相当する値は存在しない。9.18.1P5 で再確認済み。1-8 を参照 |
...
### 1-6. mandatory + synchronous モードにおける遮断の不成立
...
| 問い | 状態 |
|------|------|
| 他の ONTAP バージョンでも同じか | 構造の側は `9.18.1P5` で再確認済み。測定の側は再実行していない。1-8 を参照 |
...
| `mandatory` モードで操作を遮断できる | 再測定していない。継承 |

したがって `9.18.1P5` でも設定を S3 に及ばせることはできず、この穴は維持されると見込まれる。
これは構造からの推論であって、挙動の 2 度目の測定ではない。両者は交換可能ではなく、その区別を
保つためにこの記録は存在している。
