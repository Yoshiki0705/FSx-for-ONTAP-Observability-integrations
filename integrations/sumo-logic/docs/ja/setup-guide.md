...
```bash
bash integrations/sumo-logic/scripts/cleanup.sh          # スタックのみ
bash integrations/sumo-logic/scripts/cleanup.sh --all    # + シークレット・レイヤー・S3 テストデータ
bash integrations/sumo-logic/scripts/cleanup.sh --all -y  # 非対話
```
