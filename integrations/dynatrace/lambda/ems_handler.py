"""FSx for ONTAP EMS event shipper for Dynatrace.
...
Dynatrace payload format and endpoint.
...
DYNATRACE_ENV_URL = os.environ.get("DYNATRACE_ENV_URL", "")

MAX_BATCH_BYTES = 1 * 1024 * 1024  # Dynatrace Log Ingest accepts 1MB per request
...
HTTP = build_pool()
CREDENTIAL = SecretCache(API_KEY_SECRET_ARN, json_keys=("api_token", "token", "api_key"))


def _format(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Format normalized EMS events for Dynatrace.
    ...
    for event in events:
        out.append({
            "content": event.get("message") or json.dumps(event, default=str),
            "log.source": SOURCE_NAME,
            "timestamp": event.get("timestamp", ""),
            "severity": severity_of(event),
            "fsxn.event_name": event.get("event_name", ""),
            "fsxn.svm": event.get("svm", ""),
            "fsxn.source_node": event.get("source_node", ""),
        })
    return out
...
def _send(batch: list[dict[str, Any]], cred: str) -> bool:
    ...
    headers = {
        ...
        "Authorization": f"Api-Token {cred}",
    }
...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error("Failed to retrieve the API token: %s", str(e))
