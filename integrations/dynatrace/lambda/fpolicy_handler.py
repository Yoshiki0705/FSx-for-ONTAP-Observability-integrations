"""FSx for ONTAP FPolicy event shipper for Dynatrace.
...
remains here is the Dynatrace payload format and endpoint.
"""
...
def _format(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Format normalized FPolicy events for Dynatrace.
    ...
    for e in events:
        out.append({
            "content": json.dumps(e["raw"], default=str),
            "log.source": SOURCE_NAME,
            "timestamp": e["timestamp"],
            "severity": "info",
            "fsxn.operation": e["operation_type"],
            "fsxn.path": e["file_path"],
            "fsxn.user": e["user"],
            "fsxn.client_ip": e["client_ip"],
            "fsxn.svm": e["svm"],
            "fsxn.protocol": e["protocol"],
        })
    return out
