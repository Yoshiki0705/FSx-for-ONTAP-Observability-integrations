"""FSx for ONTAP FPolicy event shipper for Elastic.
...
remains here is the Elastic payload format and endpoint.
"""
...
INDEX_PREFIX = os.environ.get("INDEX_PREFIX", "fsxn-fpolicy")
...
def _format(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Format normalized FPolicy events for Elastic.
    ...
    for e in events:
        out.append({
            ...
            "event": {"code": e["operation_type"], "kind": "event",
                      "category": ["file"], "type": ["access"]},
            "user": {"name": e["user"]},
            "source": {"ip": e["client_ip"]},
            "file": {"path": e["file_path"]},
            "fsxn": {"svm": e["svm"], "operation": e["operation_type"],
                     "protocol": e["protocol"], "volume": e["volume"]},
