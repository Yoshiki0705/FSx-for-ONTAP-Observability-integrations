"""FSx for ONTAP EMS event shipper for Honeycomb.
...
Honeycomb payload format and endpoint.
...
HONEYCOMB_DATASET = os.environ.get("HONEYCOMB_DATASET", "fsxn-ems")
...
def _format(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Format normalized EMS events for Honeycomb.
    ...
    for event in events:
        entry: dict[str, Any] = {
            "data": {
                ...
                **{f"param.{k}": v for k, v in (event.get("parameters") or {}).items()},
            }
        }
...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error("Failed to retrieve the ingest key: %s", str(e))
