#!/bin/bash
# Deploy Honeycomb integration stacks for FSx for ONTAP.
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "  HONEYCOMB_SECRET_ARN    Secrets Manager ARN"
      echo "  S3_ACCESS_POINT_ARN   FSx for ONTAP S3 Access Point ARN"
      echo "  S3_BUCKET_NAME        S3 bucket name"
      echo "  HONEYCOMB_DATASET      Honeycomb dataset name (default: fsxn-audit)"
      echo ""
...
STACK_PREFIX="${STACK_PREFIX:-fsxn-honeycomb}"
HONEYCOMB_SECRET_ARN="${HONEYCOMB_SECRET_ARN:-}"
S3_ACCESS_POINT_ARN="${S3_ACCESS_POINT_ARN:-}"
S3_BUCKET_NAME="${S3_BUCKET_NAME:-}"
HONEYCOMB_DATASET="${HONEYCOMB_DATASET:-fsxn-audit}"
HONEYCOMB_API_URL="${HONEYCOMB_API_URL:-https://api.honeycomb.io}"
...
aws cloudformation deploy \
  ...
  --parameter-overrides \
    ...
    HoneycombApiKeySecretArn="${HONEYCOMB_SECRET_ARN}" \
    S3BucketName="${S3_BUCKET_NAME}" \
    HoneycombDataset="${HONEYCOMB_DATASET}" HoneycombApiUrl="${HONEYCOMB_API_URL}" \
    LogLevel="${LOG_LEVEL}" \
...
if [ "$DEPLOY_MODE" = "all" ]; then
  if [ -f "${INTEGRATION_DIR}/lambda/ems_handler.py" ]; then
    ...
    aws cloudformation deploy \
      ...
      --parameter-overrides \
        HoneycombApiKeySecretArn="${HONEYCOMB_SECRET_ARN}" \
        HoneycombApiUrl="${HONEYCOMB_API_URL}" HoneycombDataset="${HONEYCOMB_DATASET}" LogLevel="${LOG_LEVEL}" \
        "${ALARM_PARAMS[@]+"${ALARM_PARAMS[@]}"}" \
      --no-fail-on-empty-changeset
  ...
  if [ -f "${INTEGRATION_DIR}/lambda/fpolicy_handler.py" ]; then
    ...
    aws cloudformation deploy \
      ...
      --parameter-overrides \
        HoneycombApiKeySecretArn="${HONEYCOMB_SECRET_ARN}" \
        HoneycombApiUrl="${HONEYCOMB_API_URL}" HoneycombDataset="${HONEYCOMB_DATASET}" EventBusName="${FPOLICY_EVENT_BUS}" LogLevel="${LOG_LEVEL}" \
...
zip -q -j /tmp/honeycomb-handler.zip handler.py "${SHARED_PYTHON_DIR}/ontap_audit_parser.py"
aws lambda update-function-code --function-name "${STACK_PREFIX}-integration-shipper" \
  --zip-file fileb:///tmp/honeycomb-handler.zip --region "${AWS_REGION}" > /dev/null
rm -f /tmp/honeycomb-handler.zip
...
echo "=== Done === Check Honeycomb dataset 'fsxn-audit' for new events"
