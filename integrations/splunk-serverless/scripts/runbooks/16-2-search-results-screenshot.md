...
| フィールド | 説明 | 必須 |
|-----------|------|------|
| `host` | SVM 名（例: `svm-prod-01`） | ✅ |
| `source` | ソース識別子（例: `fsxn-observability`） | ✅ |
| `sourcetype` | `fsxn:ontap:audit` | ✅ |
| `index` | `fsxn_audit` | ✅ |
| `event_type` | イベント種別 | ✅ |
| `user` | 操作ユーザー | ✅ |
| `operation` | 操作種別（例: `Read`, `Write`） | ✅ |
| `path` | ファイルパス | ✅ |
| `result` | 操作結果（例: `Success`, `Failure`） | ✅ |
| `svm` | SVM 名 | ✅ |
...
# ファイル名（YYYYMMDD は撮影日）
splunk-search-results-YYYYMMDD.png

# 例: 2026年1月20日に撮影した場合
docs/screenshots/splunk/splunk-search-results-20260120.png
```
...
```bash
# 500KB 以下であることを確認
ls -la docs/screenshots/splunk/splunk-search-results-*.png

# 500KB を超える場合はリサイズ
sips --resampleWidth 1280 docs/screenshots/splunk/splunk-search-results-YYYYMMDD.png
```
...
- [ ] ファイル名が命名規約に準拠（`splunk-search-results-YYYYMMDD.png`）
- [ ] ファイルサイズが 500KB 以下
- [ ] マスキング処理が完了
