"""FSx for ONTAP FPolicy event shipper for New Relic.
...
remains here is the New Relic payload format and endpoint.
"""
...
INTAKE_URL = os.environ.get(
    "NEW_RELIC_ENDPOINT", "https://log-api.newrelic.com/log/v1"
)

MAX_BATCH_BYTES = 1 * 1024 * 1024  # New Relic accepts 1MB per request
...
HTTP = build_pool()
CREDENTIAL = SecretCache(API_KEY_SECRET_ARN, json_keys=("license_key", "NR_LICENSE_KEY", "api_key"))


def _format(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Format normalized FPolicy events for New Relic.
    ...
    for e in events:
        entry: dict[str, Any] = {
            "message": (
                f"FPolicy: {e['operation_type']} {e['file_path']} "
                f"by {e['user']} from {e['client_ip']}"
            ),
            "attributes": {
                "source": SOURCE_NAME,
...
def _send(batch: list[dict[str, Any]], cred: str) -> bool:
    ...
    body = gzip.compress(json.dumps(batch, default=str).encode("utf-8"))
    headers = {"Content-Type": "application/gzip", "Api-Key": cred}
    return post_with_retry(HTTP, INTAKE_URL, body, headers, logger)
